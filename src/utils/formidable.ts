...
const determineUploadDirectory = (req: Request): string => {
    ...
    if (req.url.includes('document') || req.url.includes('employer')) return COMPANY_DOCUMENTS_DIR;
...
// Validate file type
const isValidFileType = (mimetype: string|null|undefined, allowedTypes: string[]): boolean => {
    return mimetype ? allowedTypes.includes(mimetype) : false;
};

// Handle base64 image upload
const handleBase64Upload = (base64Data: string, uploadDir: string, prefix = 'logo'): string | null => {
    try {
...
export const handleFileUpload = (
    ...
    return new Promise((resolve, reject) => {
        ...
        form.parse(req, (err, fields, files) => {
            ...
            if (fields.logo && Array.isArray(fields.logo) && fields.logo[0]?.startsWith('data:image')) {
            ...
            const fileKeys = ['profilePicture', 'resumeFile', 'logo', 'document','postImage'];
...
// Utility to retrieve absolute file path
